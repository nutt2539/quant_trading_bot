...
def send_hourly_portfolio_summary() -> bool:
    ...
    try:
        ...
        s_pnl = unified_pnl['stock_pnl']
        c_pnl = unified_pnl['crypto_pnl']
        f_pnl = unified_pnl['forex_pnl']
        ...
        # 3. Forex System
        f_sign = "+" if f_pnl['total_pnl_thb'] >= 0 else ""
        msg += f"💱 [3. พอร์ต Forex & ทองคำ 24/5]\n"
        msg += f"• มูลค่าพอร์ต: ฿{f_pnl['current_equity']:,.2f}\n"
        msg += f"• เงินสดคงเหลือ: ฿{f_pnl['cash_balance_thb']:,.2f}\n"
        msg += f"• กำไร/ขาดทุน: {f_sign}฿{f_pnl['total_pnl_thb']:,.2f} ({f_sign}{f_pnl['total_pnl_pct']:.2f}%)\n"
        f_pos = f_pnl['active_positions_detail']
        if f_pos:
            holdings_str = ", ".join([f"{p['ชื่อสินทรัพย์']} ({p['กำไร/ขาดทุน (%)']})" for p in f_pos])
            msg += f"• ถือครอง ({len(f_pos)}): {holdings_str}\n"
        else:
            msg += f"• ถือครอง: ถือเงินสด 100%\n"
